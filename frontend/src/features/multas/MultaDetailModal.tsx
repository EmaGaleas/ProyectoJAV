...
export function MultaDetailModal({ record, onClose }: Props) {
  ...
  const direccionFormateada = `${ubicacion?.calle}, Bloque ${ubicacion?.codigoBloque}, Lote ${ubicacion?.loteCasa}`;
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        const data = await getUserMultado(record.idUsuario);
        setUbicacion(data.domicilios[0]);
        console.log(data.domicilios[0]);
      } catch (error) {
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <div className="flex flex-col gap-5">
              ...
              <InfoSection title="Infractor" icon={<User size={13} />}>
                <UniformGrid cols={2}>
                  <InfoField
                    label="Persona multada"
                    value={record.nombreUsuario}
                  />
