...
export function MultasTable({ records, page, onPage, onDetails }: Props) {
  ...
  const headers = ["Código", "Persona multada", "DNI", "Tipo de multa", "Fecha", "Monto", ""];
        ...
        <div style={{ minWidth: MIN_W }}>
          ...
          <div className="flex flex-col flex-1">
            {slots.map((r, i) => (
              ...
              >
                {r ? (
                  <>
                    <Cell text={r.codigoMulta} bold />
                    <Cell text={r.personaMultada} />
                    <Cell text={r.dni} />
                    <Cell text={r.tipoMulta} />
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-[rgba(0,0,0,0.06)] shrink-0" style={{ background: "#FAFAFA" }}>
            <span style={{ fontSize: 11, color: "#8EBFA3" }}>
              {records.length === 0 ? "Sin resultados" : `${safePage * PAGE_SIZE + 1}–${Math.min((safePage + 1) * PAGE_SIZE, records.length)} de ${records.length} registros`}
            </span>
            <div className="flex items-center gap-0.5">
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <PageBtn key={i} onClick={() => onPage(i + 1)} disabled={false} active={i === safePage}>{i + 1}</PageBtn>
              ))}
...
function Cell({ text, bold }: { text: string; bold?: boolean }) {
  return (
    <div className="px-3 flex items-center overflow-hidden">
      <span className="truncate" style={{ fontSize: 13, color: "#1A1A1A", fontWeight: bold ? 600 : 400 }}>{text}</span>
    </div>
  );
}
...
function PageBtn({ onClick, disabled, active, children }: { onClick: () => void; disabled: boolean; active?: boolean; children: React.ReactNode }) {
  return (
    <button
      ...
      style={{
        width: 26, height: 26, fontSize: 12, fontWeight: active ? 700 : 400,
        background: active ? "#308C58" : "transparent", color: active ? "#fff" : "#555",
      }}
    >
      {children}
    </button>
