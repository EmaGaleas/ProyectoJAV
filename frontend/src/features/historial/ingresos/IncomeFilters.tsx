export type TipoIngreso  = 'Mensualidad' | 'Multa' | 'Conexión'
export type EstadoIngreso = 'Pagado' | 'Pendiente' | 'Vencido'
...
export const DEFAULT_FILTERS: Filters = { tipoIngreso: '', estado: '', dateFrom: '', dateTo: '' }
...
export function IncomeFilters({ filters, onChange, onApply }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-[rgba(0,0,0,0.07)] p-5 flex flex-col h-full" style={{ minWidth: 220 }}>
      ...
      {/* Tipo de ingreso */}
      <FilterGroup label="Tipo de ingreso">
        {(['Mensualidad', 'Multa', 'Conexión'] as TipoIngreso[]).map(type => (
          <CheckRow key={type} label={type} checked={filters.tipoIngreso === type}
            onChange={() => set('tipoIngreso', filters.tipoIngreso === type ? '' : type)} />
        ))}
      </FilterGroup>

      {/* Estado */}
      <FilterGroup label="Estado">
        {(['Pagado', 'Pendiente', 'Vencido'] as EstadoIngreso[]).map(s => (
          <CheckRow key={s} label={s} checked={filters.estado === s}
            onChange={() => set('estado', filters.estado === s ? '' : s)} />
        ))}
      </FilterGroup>
