...
// Utilidad simple para formatear la fecha a un formato legible
...
const DEFAULT_REPORTES_FILTERS: ReportesFilterValues = { 
  search: '', 
  dateFrom: '', 
  dateTo: '' 
};
...
export function ReportesFilters({ filters, onChange }: Props) {
  const set = (key: keyof ReportesFilterValues, value: string) =>
    onChange({ ...filters, [key]: value });

  const hasAny = filters.search || filters.dateFrom || filters.dateTo;
  const handleClear = () => onChange(DEFAULT_REPORTES_FILTERS);
  ...
  return (
    ...
    >
      ...
      <FilterGroup label="Buscar">
        <textarea
          placeholder="Búsqueda por residente, bloque, DNI..."
